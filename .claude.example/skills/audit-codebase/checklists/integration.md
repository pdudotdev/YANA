# Integration Point Checklist — YANA
...
| `load_intent()` prefix fallback | Does it try `yana-` prefix first, then fall back to `dblcheck-`? Verify both code paths exist. |
...
| Credential path per cli_style | Is the Vault path per cli_style (`yana/router{cli_style}`) correct for all platforms, or does it use a shared path? |
