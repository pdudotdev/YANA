...
log = logging.getLogger("yana.netbox")
...
def load_intent() -> dict | None:
    ...
    Expects config contexts named 'yana-<device>' (per router) and
    optionally 'yana-global' (containing autonomous_systems data).
    Falls back to 'dblcheck-<device>' prefix if yana prefix not found.
    ...
    try:
        ...
        nb = pynetbox.api(url, token=token)
        nb.http_session.timeout = (5, 15)
        # Try yana prefix first, fall back to dblcheck prefix
        contexts = list(nb.extras.config_contexts.filter(name__isw="yana-"))
        prefix = "yana-"
        if not contexts:
