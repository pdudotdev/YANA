...
As the netKB document corpus grows (adding BGP, EIGRP, STP, and other protocols), **retrieval precision degrades** — the proportion of truly relevant chunks in the top-k results decreases, even though recall (finding *some* relevant result) may remain acceptable.
...
Current `vendor` and `topic` filters narrow the search space, but there is no `protocol` filter. A query filtered to `topic: "rfc"` still searches across ALL RFC documents (OSPF, BGP, EIGRP, etc.).
...
- User: *"Why is router A2A not forming an OSPF adjacency with D1C?"*
- Sub-queries:
  - `"OSPF adjacency requirements RFC 2328"` (protocol theory)
  - `"A2A device inventory platform"` (inventory lookup)
  - `"A2A OSPF area configuration intent"` (design intent)
  - `"D1C OSPF area configuration intent"` (design intent)
...
| Inventory | 1200+ chars | Tabular; keep device records together |
