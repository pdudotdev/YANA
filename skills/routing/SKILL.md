...
> **Step 0 — Longest-prefix-match override:** Before checking any policy, verify no more-specific route exists: `get_routing(device, "ip_route")` and inspect for a /32 host route or more-specific subnet covering the destination. A more-specific route overrides ALL policy, cost, and AD calculations. If an unexpected specific route is present, that is the root cause — stop here.

    get_routing(device, "policy_based_routing")
    get_routing(device, "route_maps")
    get_routing(device, "prefix_lists")
    get_routing(device, "ip_route")
...
`policy_based_routing` returns only the **interface binding** — which interface has `ip policy route-map X` applied. It does not show the route-map logic. Always follow with two more queries:
    ...
    get_routing(device, "policy_based_routing")   → identifies which interface has PBR
    get_routing(device, "route_maps")             → shows match/set clauses for that route-map
    get_routing(device, "access_lists")           → shows ACL referenced in the match clause
...
Route-maps applied at redistribution points can modify metrics, biasing path preference on downstream devices. Prefix-lists used as distribute-lists or in redistribution route-maps can block routes from being accepted into or advertised from the RIB — making them absent on downstream routers.
    ...
    get_routing(device, "route_maps")     → check for set metric or deny sequences
    get_routing(device, "prefix_lists")   → check permit/deny entries for the affected prefix
...
**Distribute-list filtering** (`distribute-list` under `router ospf`) is a special case: it blocks routes from entering the RIB without removing them from the LSDB. Symptom: `get_ospf(device, "database")` shows the LSA but `get_routing(device, "ip_route")` returns no matching entry. See the OSPF skill's "Distribute-List Filtering" section for the full investigation flow — use `get_routing(device, "access_lists")` or `get_routing(device, "prefix_lists")` to inspect the filter logic.

> For BGP attribute manipulation (`set local-preference`, `set weight`, `set as-path prepend`), these are outside YANA's current tool scope — advise the operator to inspect BGP configuration on-device.
...
When two equal-cost paths are expected but traffic consistently uses the same one:
    ...
    get_routing(device, "ip_route")   → must show 2+ equal-cost entries for true ECMP
...
3. **Check BGP `maximum-paths`** — BGP defaults to 1 path (no ECMP). Unlike OSPF (default 4), BGP requires explicit configuration. BGP maximum-paths verification requires on-device access — outside YANA's current tool scope.
...
```
get_routing(device, "ip_route")   → check [AD/metric] field in RIB output
```
...
> **Low priority** — only investigate NAT after all routing, adjacency, and policy checks pass.

When the breaking hop is a **NAT_EDGE** device (E1C, E2C per INTENT.json) and all routing, neighbor, and policy checks are clean, NAT translation issues are possible. NAT investigation requires on-device access — outside YANA's tool scope. Advise the operator to run `show ip nat translations` and `show ip nat statistics` directly on the device.
...
| `ip_route` | Full IP routing table — inspect for specific prefix entries and [AD/metric] values |

For redistribution configuration (active redistribute statements), use `get_ospf(device, "config")` — redistribution is configured under the routing protocol block and is visible in the running config section.

---

## Diagnosis Checklist

- [ ] `get_routing(device, "policy_based_routing")` — no unexpected `ip policy` bindings on ingress interfaces
- [ ] `get_routing(device, "route_maps")` — no `deny` sequences or `set metric` clauses unexpectedly biasing the path
- [ ] `get_routing(device, "prefix_lists")` — affected prefix is covered by `permit`, not `deny`
- [ ] `get_routing(device, "ip_route")` — correct number of next-hops installed in RIB; expected [AD/metric] value
